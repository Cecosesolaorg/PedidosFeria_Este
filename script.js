...
function saveToLocalStorage() {
    ...
    // Guardar timestamp para el Responsable 2 (5 AM reset)
    localStorage.setItem('r2_timestamp', new Date().getTime().toString());
}

function checkResponsable2Persistence() {
    const savedTimestamp = localStorage.getItem('r2_timestamp');
    if (!savedTimestamp) return false;

    const savedDate = new Date(parseInt(savedTimestamp, 10));
    const nowDate = new Date();

    // Lógica de "Día Lógico" empezando a las 5 AM
    const getLogicalDay = (date) => {
        const d = new Date(date);
        d.setHours(d.getHours() - 5);
        return d.toDateString();
    };

    if (getLogicalDay(nowDate) !== getLogicalDay(savedDate)) {
        return false; // Expirado
    }
    return true; // Sigue vigente
}
...
function checkWeeklyReset() {
    ...
    if (!lastResetDate || lastResetDate < currentSunday) {
        selectedProducts = [];
        // Responsable 1 es permanente, solo limpiamos Responsable 2 si ha pasado el tiempo semanal
        // Aunque la regla diaria de las 5 AM es más estricta para el R2.
...
function loadFromLocalStorage() {
    ...
    if (savedResponsables) {
        const parsed = JSON.parse(savedResponsables);
        responsables[0] = parsed[0] || '';
        // Cargar Responsable 2 solo si no ha expirado (5 AM rule)
        if (checkResponsable2Persistence()) {
            responsables[1] = parsed[1] || '';
        } else {
            responsables[1] = '';
        }
    }
...
clearAllBtn.addEventListener('click', () => {
    if (selectedProducts.length === 0 && customProducts.length === 0) return;

    // 1. Limpiar productos seleccionados
    selectedProducts = [];
    hasShared = false;

    // 2. Eliminar productos manuales de la lista principal y directOrder
    customProducts.forEach(cp => {
        // Eliminar de products
        const pIndex = products.findIndex(p => p.id === cp.id);
        if (pIndex !== -1) products.splice(pIndex, 1);

        // Eliminar de directOrderProducts
        const doIndex = directOrderProducts.findIndex(p => p.id === cp.id);
        if (doIndex !== -1) directOrderProducts.splice(doIndex, 1);

        // Eliminar de mappings de empresas
        Object.keys(companyProductMapping).forEach(company => {
            const mIndex = companyProductMapping[company].indexOf(cp.name);
            if (mIndex !== -1) {
                companyProductMapping[company].splice(mIndex, 1);
            }
        });
    });

    // 3. Resetear array de manuales
    customProducts = [];
    ...
    refreshCompanyProductViews();
    renderCompanyList();
    if (filters.directOrder) renderDirectOrderSearchResults();

    showMessage('Lista y productos manuales vaciados.', '#fca5a5', '#991b1b');
});
...
window.addEventListener('load', () => {
    responsable1Input.value = responsables[0] || '';
    responsable2Input.value = responsables[1] || ''; // Respetar el valor cargado (que ya pasó el check de 5am)
